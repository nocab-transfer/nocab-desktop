...
class Server {
  ...
  Future<void> acceptRequest(ShareRequest request, Socket socket) async {
    ShareResponse _shareResponse = ShareResponse(response: true);
    socket.write(base64.encode(utf8.encode(json.encode(_shareResponse.toJson()))));
    socket.close();
  ...
  Future<void> rejectRequest(ShareRequest request, Socket socket, [String? message]) async {
    ShareResponse _shareResponse = ShareResponse(response: false, info: message ?? "User rejected request");
    socket.write(base64.encode(utf8.encode(json.encode(_shareResponse.toJson()))));
    socket.close();
  }
