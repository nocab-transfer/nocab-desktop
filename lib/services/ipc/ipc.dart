...
class IPC {
  ...
  static Future<bool> isNoCabAlreadyRunning() async {
    ...
    return await Process.run('tasklist', ['/nh', '/fo', 'csv', '/fi', 'imagename eq nocab_desktop.exe', '/fi', 'PID ne $pid']).then((ProcessResult results) {
  ...
  Future<void> watch(Function(List<String> args)? onData) async {
    ...
    if (await watchFile.parent.exists()) await watchFile.parent.delete(recursive: true);
  ...
  static String getBasePath() {
    if (Platform.isWindows) return p.join(Platform.environment['APPDATA']!, 'NoCab IPC');
